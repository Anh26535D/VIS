...
import numpy as np
from Base.WelcomeToTheDungeon_v2_draft import _env
IMG_PATH = "E:/vis/2023-05/ENV/Base/WelcomeToTheDungeon_v2_draft/assets/"
w_bg, h_bg = 1680, 720
BG_SIZE = (w_bg, h_bg)
c_w, c_h = 80, 112
CARD_SIZE = (c_w, c_h)
...
class Sprites:
    ...
    def __init__(self) -> None:
        self.background = Image.open(IMG_PATH + "bg.png").convert("RGBA").resize(BG_SIZE)
        self.background.putalpha(80)
        
        self.aidcard = Image.open(IMG_PATH + "aid_card.png").convert("RGBA").resize(CARD_SIZE)
        
        self.figureImgs = []
        for fig in self.figures:
            imgPath = IMG_PATH + fig + ".png"
            self.figureImgs.append(Image.open(imgPath).convert("RGBA").resize(CARD_SIZE))

        self.barbarianEquipImgs = []
        for equip in self.barbarianEquips:
            imgPath = IMG_PATH + "barbarian_" + equip + ".png"
            self.barbarianEquipImgs.append(Image.open(imgPath).convert("RGBA").resize(CARD_SIZE))

        self.mageEquipImgs = []
        for equip in self.mageEquips:
            imgPath = IMG_PATH + "mage_" + equip + ".png"
            self.mageEquipImgs.append(Image.open(imgPath).convert("RGBA").resize(CARD_SIZE))

        self.monsterImgs = []
        for monster in self.monsters:
            imgPath = IMG_PATH + "monster_" + monster + ".png"
            self.monsterImgs.append(Image.open(imgPath).convert("RGBA").resize(CARD_SIZE))            

sprites = Sprites()

def getFont(size):
    return ImageFont.truetype(r'E:/vis/2023-05/ENV/ImageFonts/arial.ttf', size)
...
def addEquips(background, state):
    ...
    if state[53]==0 and state[61]==0:
        ...
        for equip in equips[0]:
            background.paste(equip.resize((int(c_w*1.5), int(c_h*1.5))), (int(w_bg*0.1) + i*150 ,int(h_bg*0.3)))
            i += 1

        background.paste(figures[1].resize((f_w, f_h)), (f_x, f_y + f_h + 10))
        i=1
        for equip in equips[1]:
            background.paste(equip.resize((int(c_w*1.5), int(c_h*1.5))), (int(w_bg*0.1) + i*150 ,int(h_bg*0.6)))
            i += 1

        background = addText(
            background, 
            'Please choose a hero to start the round',
            (int(w_bg*0.1), 10)
            )
    else:
        f_w, f_h = int(c_w*1.7), int(c_h*1.7)
...
def addAidCard(background, state):
    if state[53]==1 or state[61]==1:
        background.paste(sprites.aidcard.resize((int(c_w*2.0), int(c_h*1.5))), (500,600))
    return background

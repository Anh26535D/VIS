from CompaniesmarketcapCrawler import CompaniesmarketcapCrawler
import os
import pandas as pd

IGNORE_FLAG = True

base_path = r"E:\vis\vis_repo\crawl_wrapper\companiesmarketcap"
list_symbols_path = os.path.join(base_path, "list_companies.csv")
save_path = os.path.join(base_path, "dividend")
...
crawler = CompaniesmarketcapCrawler()
...
for i, symbol in enumerate(symbols):
    ...
    dividend_and_split_data, log_error = crawler.getDividendAndSplitData(symbol=symbol)

    if log_error == "OK":
        print(f"===> Sucessfully. {dividend_and_split_data.shape}")
        save_file_path = os.path.join(save_path, f"{symbol}.csv")
        dividend_and_split_data.to_csv(save_file_path, index=False)
    else: 
